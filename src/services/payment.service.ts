...
export interface PaymentGateway {
  ...
  validateExpiry(expiryDate: string): boolean;
  validateCVV(cvv: string): boolean;
}

export class SimulatedPaymentGateway implements PaymentGateway {
  private readonly successRate = 0.95; // Reduce flakiness: ~95% success
  private readonly processingDelay = 2000; // Match test expectation (>=1500ms)
  private callCounter = 0; // ensure periodic failures for test determinism
  ...
  async processPayment(data: PaymentData): Promise<PaymentResponse> {
    ...
    if (!validation.isValid) {
      ...
      if (validation.errorCode) resp.errorCode = validation.errorCode;
      return resp;
    }
  ...
  private validatePaymentData(data: PaymentData): { isValid: boolean; message: string; errorCode?: string } {
